...
h4_to_h = []
for i in index_list:
    query_key_value.append(f"transformer.h.{i}.self_attention.query_key_value.weight")
    dense.append(f"transformer.h.{i}.self_attention.dense.weight")
    h_to_4h.append(f"transformer.h.{i}.mlp.dense_h_to_4h.weight")
    h4_to_h.append(f"transformer.h.{i}.mlp.dense_4h_to_h.weight")
                    ...
                    if eval_dataset_name == "test":
                        ...
                        for batch in tqdm(eval_dataloader):
                            with torch.no_grad():
                                ...
                                step = step + 1
                                if step == 200:
                                    np.save('probing/bloom-'+args.mode+'/all_hidden_states' + str(i) + '.npy', torch.cat(all_hidden_states, dim = 0))
                                    np.save('probing/bloom-'+args.mode+'/all_labels' + str(i) + '.npy', torch.cat(all_labels, dim = 0))
                                    all_hidden_states = []
                                    all_labels = []
                                    i = i + 1
                                    step = 1
                        ...
                        data = np.concatenate([np.load('probing/bloom-'+args.mode+'/all_hidden_states' + str(i+1) + '.npy') for i in range(19)], axis = 0)
                        label = np.concatenate([np.load('probing/bloom-'+args.mode+'/all_labels' + str(i+1) + '.npy') for i in range(19)], axis = 0)
                        ...
                        for i in index_list:
                            ...
                            h4_to_h.append(f"transformer.h.{i}.mlp.dense_4h_to_h.weight")
                        modules_list = query_key_value + dense + h_to_4h + h4_to_h
                        with open('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/bloom-modules.json', 'w+') as fs:
...
def main():
    ...
    training_args = TrainingArguments(
        output_dir='ckpt/bloom-'+args.mode+f'/{args.cur_epoch}',
        ...
        logging_dir="logs/bloom-"+args.mode,
    ...
    if args.cur_epoch == 0:
        model = BloomForCausalLM.from_pretrained("../model/"+args.model_name, torch_dtype=torch.float16)
    else:
        fs = open('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/bloom-modules.json', 'r')
        lines = fs.readlines()
        modules_list = json.loads(lines[0])["modules"]
        model = BloomForCausalLM.from_pretrained('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/ckpt/bloom-'+args.mode+f'/{args.cur_epoch-1}/checkpoint-1841', torch_dtype=torch.float16)
