...
IGNORE_INDEX = -100
index_list = [31, 27, 26, 28, 30, 29, 3, 5]
...
down_proj = []
for i in index_list:
    k_proj.append(f"model.layers.{i}.self_attn.k_proj.weight")
    v_proj.append(f"model.layers.{i}.self_attn.v_proj.weight")
    q_proj.append(f"model.layers.{i}.self_attn.q_proj.weight")
    o_proj.append(f"model.layers.{i}.self_attn.o_proj.weight")
    gate_proj.append(f"model.layers.{i}.mlp.gate_proj.weight")
    up_proj.append(f"model.layers.{i}.mlp.up_proj.weight")
    down_proj.append(f"model.layers.{i}.mlp.down_proj.weight")
modules_list = k_proj + v_proj + q_proj + o_proj + gate_proj + up_proj + down_proj
CONFIG = PretrainedConfig.from_pretrained("../model/"+args.model_name)
                    ...
                    if eval_dataset_name == "test":
                        ...
                        for batch in tqdm(eval_dataloader):
                            with torch.no_grad():
                                ...
                                step = step + 1
                                if step == 200:
                                    np.save('probing/koala-'+args.mode+'/all_hidden_states' + str(i) + '.npy', torch.cat(all_hidden_states, dim = 0))
                                    np.save('probing/koala-'+args.mode+'/all_labels' + str(i) + '.npy', torch.cat(all_labels, dim = 0))
                                    all_hidden_states = []
                                    all_labels = []
                                    i = i + 1
                                    step = 1
                        ...
                        data = np.concatenate([np.load('probing/koala-'+args.mode+'/all_hidden_states' + str(i+1) + '.npy') for i in range(19)], axis = 0)
                        label = np.concatenate([np.load('probing/koala-'+args.mode+'/all_labels' + str(i+1) + '.npy') for i in range(19)], axis = 0)
                        ...
                        for i in index_list:
                            ...
                            down_proj.append(f"model.layers.{i}.mlp.down_proj.weight")
                        modules_list = k_proj + v_proj + q_proj + o_proj + gate_proj + up_proj + down_proj
                        with open('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/koala-modules.json', 'w+') as fs:
...
def main():
    ...
    training_args = TrainingArguments(
        output_dir='ckpt/koala-'+args.mode+f'/{args.cur_epoch}',
        ...
        logging_dir="logs/koala-"+args.mode,
    ...
    if args.cur_epoch == 0:
        ...
        fs = open('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/koala-modules.json', 'r')
        lines = fs.readlines()
        modules_list = json.loads(lines[0])["modules"]
        model = LlamaForCausalLM.from_pretrained('/mnt/nas-coai-wlcb/fhwexp/prefix-tuning/ckpt/koala-'+args.mode+f'/{args.cur_epoch-1}/checkpoint-1841', torch_dtype=torch.float16)
